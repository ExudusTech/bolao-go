          ...
          <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-10 lg:grid-cols-12 gap-2">
            {fullRanking.map(({ number, count }, index) => {
              ...
              return (
                <div 
                  key={number} 
                  className={`flex flex-col items-center justify-center p-1.5 sm:p-2 rounded-lg ${bgClass} transition-transform hover:scale-105`}
                >
                  <span className="text-sm sm:text-lg font-bold">{number.toString().padStart(2, "0")}</span>
                  <span className="text-[10px] sm:text-xs opacity-80">{count}x</span>
                  <span className="text-[8px] sm:text-[10px] opacity-60">#{position}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="pt-4 border-t space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 text-xs">
              <div className="p-3 rounded bg-primary/10 border border-primary/20">
                ...
                <p className="font-mono text-sm break-all">{fullRanking.slice(0, 10).map(n => n.number.toString().padStart(2, "0")).join(", ")}</p>
              </div>
              <div className="p-3 rounded bg-accent/10 border border-accent/20">
                <p className="text-muted-foreground mb-1">11º ao 20º</p>
                <p className="font-mono text-sm break-all">{fullRanking.slice(10, 20).map(n => n.number.toString().padStart(2, "0")).join(", ")}</p>
              </div>
              <div className="p-3 rounded bg-muted/50 border">
                <p className="text-muted-foreground mb-1">21º ao 30º</p>
                <p className="font-mono text-sm break-all">{fullRanking.slice(20, 30).map(n => n.number.toString().padStart(2, "0")).join(", ")}</p>
              </div>
              <div className="p-3 rounded bg-destructive/10 border border-destructive/20">
                <p className="text-muted-foreground mb-1">❄️ Últimos 10 (menos votados)</p>
                <p className="font-mono text-sm break-all">{fullRanking.slice(-10).map(n => n.number.toString().padStart(2, "0")).join(", ")}</p>
              </div>
            </div>
            {notVoted.length > 0 && (
          ...
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
            ...
            <div className="flex flex-wrap gap-1">
              {mostVoted.slice(0, 6).map(({ number, count }) => (
                <Badge key={number} variant="secondary" className="bg-primary/20 text-primary text-xs">
            ...
            <div className="flex flex-wrap gap-1">
              {notVoted.length > 0 ? (
                notVoted.slice(0, 8).map((number) => (
                  <Badge key={number} variant="outline" className="border-accent/50 text-accent text-xs">
